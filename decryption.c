#include<stdio.h>

// AES inverse S-box
static const unsigned char inv_sbox[256] = {
    
};

// Galois Field multiplication function (needed for invMixColumns)
unsigned char gmul(unsigned char a, unsigned char b) {
    unsigned char result = 0;
    unsigned char high_bit;
    
    for (int i = 0; i < 8; i++) {
        if (b & 1) {
            result ^= a;
        }
        
        high_bit = a & 0x80;
        a <<= 1;
        if (high_bit) {
            a ^= 0x1b; // AES irreducible polynomial x^8 + x^4 + x^3 + x + 1
        }
        b >>= 1;
    }
    return result;
}

void invSubBytes(unsigned char block[16])
{
    printf("InvSubBytes called\n");
    for (int i = 0; i < 16; i++) {
        block[i] = inv_sbox[block[i]];
    }
}

void invShiftRows(unsigned char block[16])
{
    printf("InvShiftRows called\n");

    unsigned char temp;

    // row 0: no change (indices 0,1,2,3)

    // row 1: shift right by 1 (indices 4,5,6,7)
    // This undoes the left shift by 1 from encryption
    temp = block[7];
    block[7] = block[6];
    block[6] = block[5];
    block[5] = block[4];
    block[4] = temp;

    // row 2: shift right by 2 (indices 8,9,10,11)
    // This undoes the left shift by 2 from encryption
    temp = block[8];
    unsigned char temp2 = block[9];
    block[8] = block[10];
    block[9] = block[11];
    block[10] = temp;
    block[11] = temp2;

    // row 3: shift right by 3 (indices 12,13,14,15)
    // This undoes the left shift by 3 from encryption
    temp = block[12];
    block[12] = block[13];
    block[13] = block[14];
    block[14] = block[15];
    block[15] = temp;
}

void invMixColumns(unsigned char block[16])
{
    printf("InvMixColumns called\n");
    for (int c = 0; c < 4; c++) {
        int col = c * 4;
        unsigned char a0 = block[col + 0];
        unsigned char a1 = block[col + 1];
        unsigned char a2 = block[col + 2];
        unsigned char a3 = block[col + 3];

        block[col + 0] = gmul(a0, 0x0e) ^ gmul(a1, 0x0b) ^ gmul(a2, 0x0d) ^ gmul(a3, 0x09);
        block[col + 1] = gmul(a0, 0x09) ^ gmul(a1, 0x0e) ^ gmul(a2, 0x0b) ^ gmul(a3, 0x0d);
        block[col + 2] = gmul(a0, 0x0d) ^ gmul(a1, 0x09) ^ gmul(a2, 0x0e) ^ gmul(a3, 0x0b);
        block[col + 3] = gmul(a0, 0x0b) ^ gmul(a1, 0x0d) ^ gmul(a2, 0x09) ^ gmul(a3, 0x0e);
    }
}